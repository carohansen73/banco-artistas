document.addEventListener("DOMContentLoaded",()=>{const m=document.getElementById("filter-nombre"),a=document.getElementById("filter-disciplina"),s=document.getElementById("filter-genero"),B=document.getElementById("btn-limpiar"),v=document.getElementById("contador-resultados"),f=document.querySelectorAll(".tag-disc"),w=document.getElementById("btn-ver-mas"),u=document.getElementById("ver-mas-wrap"),C=document.getElementById("ver-mas-restantes"),M=9;let y;function c(){clearTimeout(y),y=setTimeout(()=>{const t=new URLSearchParams({busqueda:m.value,disciplina:a.value,genero:s.value});fetch(`/buscador-de-artistas?${t}`).then(e=>e.json()).then(e=>{l=e,i=0;const n=document.getElementById("container-artists");if(n.innerHTML="",e.length===0){n.innerHTML=`
                            <div class="col-12 text-center text-muted py-5">
                                <p>No se encontraron artistas con esos filtros.</p>
                            </div>`,u.style.setProperty("display","none","important"),p();return}h(),p()}).catch(console.error)},300)}function h(){const t=l.slice(i,i+M);t.forEach(e=>T(e)),i+=t.length,E()}function E(){const t=l.length-i;t>0?(u.style.setProperty("display","block","important"),C.textContent=`(${t} más)`):u.style.setProperty("display","none","important")}function p(){if(!v)return;const t=l.length;v.innerHTML=t===0?'<span class="contador-texto">Sin resultados</span>':`<span class="contador-numero">${t}</span>
               <span class="contador-texto">
                    artista${t!==1?"s":""}
                </span>`}function o(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function T(t){const e=document.getElementById("container-artists"),n=o(t.nombre_artistico),d=o(t.localidad),r=o(t.disciplina),L=encodeURIComponent(t.slug),A=encodeURI(t.img_perfil||""),I=(t.generos||[]).map(P=>`<span class="artista-badge genero">${o(P)}</span>`).join(""),x=(t.disciplina||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/\s+/g,"-"),H=t.img_perfil&&!t.img_perfil.includes("default")?`<img src="${A}" alt="${n}" loading="lazy">`:`<div class="artista-avatar-default">${o(t.nombre_artistico.charAt(0).toUpperCase())}</div>`,g=document.createElement("div");g.className="col-lg-4 col-md-6 col-sm-12",g.innerHTML=`
            <div class="artista-card" onclick="window.location='/artistas/${L}'">
                ...
                <div class="artista-card-body">
                    <div class="d-flex justify-content-between">
                        ...
                        ${t.disciplina?`<span class="artista-card-disciplina disc-${x}">${r}</span>`:""}
                    </div>
